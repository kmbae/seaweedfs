...
use crate::{rdma::MemoryRegion, RdmaError, RdmaResult};
...
impl SessionManager {
    ...
    pub fn new(max_sessions: usize, default_timeout: Duration) -> Self {
        info!(
            "🎯 Session manager initialized: max_sessions={}, timeout={:?}",
    ...
    pub async fn create_session(
        ...
        info!(
            "📦 Created session {}: volume={}, needle={}, size={}",
    ...
    pub async fn remove_session(&self, session_id: &str) -> RdmaResult<()> {
        self.take_session(session_id).await.map(|_| ())
    }

    /// Remove a session and return it to the caller for final resource cleanup.
    pub async fn take_session(&self, session_id: &str) -> RdmaResult<Arc<RwLock<RdmaSession>>> {
        let session = {
            let mut sessions = self.sessions.write();
        ...
        if let Some(session) = session {
            {
                ...
                info!(
                    "🗑️ Removed session {}: stats={:?}",
                    session_id, session_data.stats
                );
                ...
                if !expired_sessions.is_empty() {
                    ...
                    for session_id in expired_sessions {
                        if let Some(session) = sessions_guard.remove(&session_id) {
                            let session_data = session.read();
                            info!(
                                "🗑️  Cleaned up expired session: {} (volume={}, needle={})",
