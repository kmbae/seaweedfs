...
impl MemoryPool {
    ...
    pub fn new(max_pool_size: usize, max_total_memory: usize) -> Self {
        info!(
            "🧠 Memory pool initialized: max_pool_size={}, max_total_memory={} bytes",
    ...
    pub fn allocate(&self, size: usize) -> RdmaResult<Arc<RwLock<PooledBuffer>>> {
        ...
        debug!(
            "🆕 Allocated new buffer: size={}, total_allocated={}",
            pool_size, new_total
        );
    ...
    pub fn cleanup_old_buffers(&self, max_age: std::time::Duration) {
        ...
        if cleaned_count > 0 {
            ...
            info!(
                "🧹 Cleaned up {} old buffers, freed {} bytes",
...
impl RdmaMemoryManager {
    ...
    fn allocate_hugepage_buffer(&self, size: usize) -> RdmaResult<RdmaBuffer> {
        #[cfg(target_os = "linux")]
        {
            ...
            let aligned_size =
                (size + self.config.hugepage_size - 1) & !(self.config.hugepage_size - 1);
            ...
            match addr {
                Ok(addr) => {
                    ...
                    info!(
                        "🔥 Allocated hugepage buffer: addr=0x{:x}, size={}",
                        addr_u64, aligned_size
                    );
                    Ok(RdmaBuffer::HugePage {
                        addr: addr_u64,
                        size: aligned_size,
                    })
                }
    ...
    pub fn deallocate_buffer(&self, buffer: RdmaBuffer) -> RdmaResult<()> {
        match buffer {
            RdmaBuffer::Pool { buffer, .. } => self.pool.deallocate(buffer),
            ...
            RdmaBuffer::HugePage { addr, size } => {
                ...
                debug!(
                    "🗑️ Deallocated hugepage buffer: addr=0x{:x}, size={}",
                    addr, size
                );
                Ok(())
            }
...
pub enum RdmaBuffer {
    ...
    Mmap { addr: u64, size: usize },
    /// HugePage buffer
    HugePage { addr: u64, size: usize },
}
...
impl RdmaBuffer {
    ...
    pub fn addr(&self) -> u64 {
        match self {
            Self::Pool { buffer, .. } => buffer.read().as_ptr() as u64,
            Self::Mmap { addr, .. } => *addr,
    ...
    pub fn to_vec(&self) -> Vec<u8> {
        match self {
            Self::Pool { buffer, .. } => buffer.read().as_slice().to_vec(),
            Self::Mmap { addr, size } => unsafe {
                let slice = std::slice::from_raw_parts(*addr as *const u8, *size);
                slice.to_vec()
            },
            Self::HugePage { addr, size } => unsafe {
                let slice = std::slice::from_raw_parts(*addr as *const u8, *size);
                slice.to_vec()
            },
        }
